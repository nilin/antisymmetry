...
def data_only(files,colors,subfolder):
	...
	savename=' '.join(files)
	plt.savefig('plots_/'+subfolder+savename+'.pdf')
...
def data_and_proxy(files,proxies,colors,markers,**kwargs):
	...
	plt.savefig('plots_/'+savename+'.pdf')
...
data_only(['osc','HS','ReLU','exp','tanh','DReLU'],['k','g','b','r','m','c'],'abbrev_separated/')
data_only(['osc','HS','ReLU','exp','tanh','DReLU'],['k','g','b','r','m','c'],'abbrev/')
